...
char* stb_include_string(
    const char* str,
    const char* inject,
    const char* path_to_includes,
    const char* filename_for_line_directive,
    char error[256]);

// Concatenate the strings 'strs' and do include-processing on the result. To free the return value,
// pass it to free()
char* stb_include_strings(
    const char** strs,
    int count,
    const char* inject,
    const char* path_to_includes,
...
// 'filename' is opened directly; 'path_to_includes' is not used. To free the return value, pass it
// to free()
char* stb_include_file(
    const char* filename,
    const char* inject,
    const char* path_to_includes,
    char error[256]);
...
static include_info* stb_include_append_include(
    include_info* array,
    int len,
    int offset,
    int end,
            ...
            if (0 == strncmp(s, "include", 7) && stb_include_isspace(s[7])) {
                ...
                if (*s == '"') {
                    ...
                    if (*t == '"') {
                        ...
                        list = stb_include_append_include(
                            list,
                            inc_count++,
                            (int)(start - text),
                            (int)(s - text),
                ...
                while (*s != '\r' && *s != '\n' && *s != 0)
                    ++s;
                list = stb_include_append_include(
                    list, inc_count++, (int)(start - text), (int)(s - text), NULL, line_count + 1);
            }
        }
        while (*s != '\r' && *s != '\n' && *s != 0)
            ++s;
...
#endif
        if (inc_list[i].filename == 0) {
            if (inject != 0) text = stb_include_append(text, &textlen, inject, strlen(inject));
        } else {
            char* inc;
...
char* stb_include_file(
    const char* filename,
    const char* inject,
    [[maybe_unused]] const char* path_to_includes,
    char error[256])
{
    size_t len;
    char* result;
